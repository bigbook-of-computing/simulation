...
**Sample Output:**
```python
--- Heterogeneous Agent Order Flow Analysis ---
Scenario: Price P_t = 110.00 (Fundamental Value P_fund = 100.00)

---

1. Fundamentalist Actions (Stabilizing / Negative Feedback):
   Action: -1.0 (Sell, as P_t > P_fund)
   Order Flow O_fund: -499.0 (Attempting to push price DOWN)

2. Chartist Actions (Destabilizing / Positive Feedback):
   Action: 1.0 (Buy, as trend is positive)
   Order Flow O_chart: 498.0 (Attempting to push price UP)

3. Net Order Flow:
   Total Order Flow (O_t): -1.0
   Net Order Flow per Agent: -0.001
...
```python
--- Fat Tail Analysis Summary ---
Simulated Volatility (StDev): 0.0003
Total Steps: 10000

---

Simulated P(|R| > 2σ) (Approx): 0.0031
Theoretical Gaussian P(|R| > 2σ): 0.0455
Ratio (Simulated/Gaussian): 0.1x
