...
**Python Implementation: Parallel Tempering Framework**

```python
import numpy as np

def ising_energy_2d(spins, J=1.0):
    """Total energy of a 2D Ising configuration (open boundary)."""
    return -J * (np.sum(spins[:-1] * spins[1:]) + np.sum(spins[:, :-1] * spins[:, 1:]))

def metropolis_sweep(spins, beta, J=1.0):
    """One full sweep (N×N attempted flips) of Metropolis single-spin updates."""
    N = spins.shape[0]
    for _ in range(N * N):
...
def parallel_tempering(n_replicas=4, n_steps=500, L=8, J=1.0, seed=42):
    """
    Parallel Tempering (Replica Exchange Monte Carlo) for 2D Ising model.

    Parameters
    ----------
    n_replicas : int   – Number of replicas at different temperatures.
    n_steps    : int   – Number of Monte Carlo sweeps per replica.
    L          : int   – Linear lattice size (L×L spins).
    J          : float – Ferromagnetic coupling constant.

    Returns
    -------
    betas   : array of inverse temperatures for each replica.
    energies: final energy per replica.
    """
    np.random.seed(seed)
    betas    = np.linspace(0.30, 0.60, n_replicas)   # temperature ladder
    replicas = [np.random.choice([-1, 1], size=(L, L)) for _ in range(n_replicas)]
    energies = [ising_energy_2d(r, J) for r in replicas]
    n_swap_accept = 0

    for step in range(n_steps):
        # Independent Metropolis sweep for each replica
        for k in range(n_replicas):
            replicas[k] = metropolis_sweep(replicas[k], betas[k], J)
            energies[k] = ising_energy_2d(replicas[k], J)

        # Replica swap (even-odd scheme preserves detailed balance)
        start = 0 if step % 2 == 0 else 1
        for k in range(start, n_replicas - 1, 2):
            m = k + 1
            delta = (betas[m] - betas[k]) * (energies[m] - energies[k])
            if np.random.rand() < np.exp(delta):
                replicas[k], replicas[m] = replicas[m], replicas[k]
                energies[k], energies[m] = energies[m], energies[k]
                n_swap_accept += 1

    print(f"Parallel Tempering — n_replicas={n_replicas}, L={L}, steps={n_steps}")
    for k in range(n_replicas):
        print(f"  Replica {k+1}: β={betas[k]:.3f}  T={1/betas[k]:.3f}  E={energies[k]:.1f}")
    total_attempts = n_steps * (n_replicas - 1)
    print(f"Swap accept rate: {n_swap_accept}/{total_attempts} = {n_swap_accept/total_attempts:.2%}")
    return betas, energies

betas, energies = parallel_tempering()
```
**Sample Output:**
```
Parallel Tempering — n_replicas=4, L=8, steps=500
  Replica 1: β=0.300  T=3.333  E=-48.0
  Replica 2: β=0.400  T=2.500  E=-78.0
  Replica 3: β=0.500  T=2.000  E=-106.0
  Replica 4: β=0.600  T=1.667  E=-112.0
Swap accept rate: 237/1500 = 15.80%
```
