...
> **Python Implementation:** Euler–Maruyama simulation of GBM and Monte Carlo option pricing:

```python
import numpy as np

def euler_maruyama_gbm(S0, mu, sigma, T, n_steps, n_paths, seed=None):
    ...
    return S  # terminal prices S_T

def monte_carlo_call(S0, K, r, sigma, T, n_steps=252, n_paths=10_000):
    """Price a European call via Monte Carlo (Euler-Maruyama)."""
    S_T = euler_maruyama_gbm(S0, r, sigma, T, n_steps, n_paths)
    payoffs = np.maximum(S_T - K, 0.0)
    return np.exp(-r * T) * payoffs.mean()

# Example: price a call with S0=100, K=105, r=5%, sigma=20%, T=1yr
price = monte_carlo_call(S0=100, K=105, r=0.05, sigma=0.20, T=1.0)
print(f"Monte Carlo call price: {price:.4f}")
```
**Sample Output:**
```
Monte Carlo call price: 7.9372
```
